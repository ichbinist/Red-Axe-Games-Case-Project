...
public class RCC_Editor : Editor {
    ...
    public override void OnInspectorGUI() {
        ...
        EditorGUILayout.PropertyField(serializedObject.FindProperty("overrideBehavior"), new GUIContent("Ineffective Behavior", "Vehicle won't be affected by selected behavior in RCC Settings if enabled."), false);
        EditorGUILayout.Space();
    ...
    private void WheelSettingsTab() {
        ...
        if (GUILayout.Button("Create WheelColliders")) {
            ...
            RCC_WheelCollider[] wheelColliders = carScript.gameObject.GetComponentsInChildren<RCC_WheelCollider>();

            if (wheelColliders.Length >= 1) {

                bool deleteExistingWheelcolliders = EditorUtility.DisplayDialog("Vehicle has Wheel Colliders already!", "Vehicle has Wheel Colliders already! Delete all of them to create new Wheel Colliders.", "Delete Existing WheelColliders", "Close");
